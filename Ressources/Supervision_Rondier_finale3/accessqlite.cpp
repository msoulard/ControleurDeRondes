...
bool AccesSqlite::ConnexionBddSqlite()
{
    ...
    bddSqlite.setDatabaseName(QDir::currentPath().toLatin1() + "/BDD/Rondier_BDDRemplie.db");
...
{
    QSqlQuery requete(bddSqlite);
    requete.prepare("insert into comporte (id, id_ronde, id_pointeau, ordre, tempsmini, tempsmaxi) values (:idcomporte, :idronde, :idpointeau, :ordre, :tempsmini, :tempsmaxi);");
    requete.bindValue(":idcomporte", unComporte.idComporte);

...
QList<Pointeau *> AccesMySQL::ObtenirPointeaux()
{
    ...
    if(VerifierConnexion())
    {
        if(requete.exec("SELECT * FROM pointeaux ORDER BY alias"))
        {
            while(requete.next())
            {
                unPointeau = new Pointeau(requete.value("alias").toInt(), requete.value("idTag").toString(), requete.value("localisation").toString(), requete.value("idPointeau").toInt());
...
bool AccesMySQL::AjouterPointeau(Pointeau *nvPointeau)
{
    ...
    if(VerifierConnexion())
    {
        ...
        case 2:
            requete.prepare("INSERT INTO pointeaux (idTag, localisation, alias) VALUES (:tag, :localisation, :alias)");
            requete.bindValue(":tag", nvPointeau->ObtenirTag());
            requete.bindValue(":localisation", nvPointeau->ObtenirLieu());
            requete.bindValue(":alias", nvPointeau->ObtenirNumero());
            if(requete.exec())
            {
                valide = true;
            }
...
int AccesMySQL::ModifierPointeau(Pointeau *modifPointeau)
{
    ...
    if(VerifierConnexion())
    {
        ...
        requete.prepare("UPDATE pointeaux SET localisation=:localisation, alias=:alias WHERE idPointeau=:idPointeau");
        requete.bindValue(":localisation", modifPointeau->ObtenirLieu());
        requete.bindValue(":alias", modifPointeau->ObtenirNumero());
...
bool AccesMySQL::VerifierNumeroUnique(int numero)
{
    ...
    if(VerifierConnexion())
    {
        ...
        requete.prepare("SELECT * FROM pointeaux WHERE alias=:alias");
        requete.bindValue(":alias", numero);
        if(requete.exec())
        {
            if(requete.size() == 0)
            {
...
bool AccesMySQL::VerifierLieuUnique(QString lieu)
{
    ...
    if(VerifierConnexion())
    {
        ...
        requete.prepare("SELECT * FROM pointeaux WHERE localisation=:localisation");
        requete.bindValue(":localisation", lieu);
        if(requete.exec())
        {
            if(requete.size() == 0)
            {
...
QList<Ronde *> AccesMySQL::ObtenirRondes()
{
    ...
    if(VerifierConnexion())
    {
        if(requeteRonde.exec("SELECT * FROM rondes ORDER BY nomrondes"))
        {
            while(requeteRonde.next())
            {
                ...
                requetePointeau.prepare("SELECT pointeaux.*, tempsMin, tempsMax FROM pointeaux, associationpointeauxrondes WHERE pointeaux.idPointeau = associationpointeauxrondes.idPointeau AND idRonde = idRonde ORDER BY ordrePointeau");
                requetePointeau.bindValue(":idRonde", requeteRonde.value("idRonde"));
                if(requetePointeau.exec())
                {
                    while(requetePointeau.next())
                    {
                        unPointeau = new Pointeau(requetePointeau.value("alias").toInt(), requetePointeau.value("idTag").toString(), requetePointeau.value("localisation").toString(), requetePointeau.value("idPointeau").toInt());
                        listePointeaux.append(unPointeau);
                ...
                {
                    qDebug() << "Erreur lors de la requête \"SELECT pointeaux.*, tempsMin, tempsMax FROM pointeaux\" : " + requetePointeau.lastError().text();
                }
            }
        }
        else
...
Ronde *AccesMySQL::ObtenirRonde(int idRonde)
{
    ...
    if(VerifierConnexion())
    {
        requeteRonde.prepare("SELECT * FROM rondes WHERE idrondes=:idrondes");
    ...
    if(VerifierConnexion())
    {
        ...
        if(requete.exec())
        {
            ...
            if(requete.exec("SELECT LAST_INSERT_ID()"))
            {
                ...
                {
                    requeteAssociationPointeauxRondes.prepare("INSERT INTO associationpointeauxrondes (idRonde, idPointeau, ordrePointeau, tempsMin, tempsMax) VALUES (:idRonde, :idPointeau, :ordrePointeau, :tempsMin, :tempsMax)");
...
int AccesMySQL::ModifierRonde(Ronde *modifRonde)
{
    ...
    if(VerifierConnexion())
    {
        ...
        if(requete.exec())
        {
            ...
            {
                requete.prepare("INSERT INTO associationpointeauxrondes (idRonde, idPointeau, ordrePointeau, tempsMin, tempsMax) VALUES (:idRonde, :idPointeau, :ordrePointeau, :tempsMin, :tempsMax)");
                requete.bindValue(":idRonde",  modifRonde->ObtenirIdRonde());
...
bool AccesMySQL::VerifierNomRondeUnique(QString nomRonde)
{
    ...
    if(VerifierConnexion())
    {
        ...
        requete.prepare("SELECT * FROM rondes WHERE nomrondes=:nomrondes");
...
//        requete.prepare("INSERT INTO historiquePointeau(nomagent, nomronde, nompointeau, date, ordrepointeau, numeroronde, anomalieOrdre, anomalieTemps) values(:nom, :nomronde, :nompointeau, :date, :ordrepointeau, :numeroronde, :anomalieOrdre, :anomalieTemps)");

//        requete.bindValue(":nom", nomAgent);
...
bool AccesMySQL::VerifierNomSmartphoneUnique(QString nom)
{
    ...
    if(VerifierConnexion())
    {
        ...
        requete.prepare("SELECT * FROM smartphones WHERE nomSmartphone=:nomSmartphone");
        requete.bindValue(":nomSmartphone", nom);
        if(requete.exec())
        {
            if(requete.size() == 0)
            {
...
bool AccesMySQL::AjouterSmartphone(Smartphone nvSmartphone)
{
    ...
    if(VerifierConnexion())
    {
        ...
        {
            requete.prepare("INSERT INTO smartphones(nomSmartphone, idADB) values(:nomSmartphone, :idADB)");
            requete.bindValue(":nomSmartphone", nvSmartphone.nom);
...
Smartphone AccesMySQL::ObtenirSmartphone(QString idADB)
{
    ...
    if(VerifierConnexion())
    {
        ...
        else
        {
            if(requete.next())
            {
                smartphone.nom = requete.value("nomSmartphone").toString();
...
QList<Smartphone> AccesMySQL::ObtenirListeSmartphones()
{
    ...
    if(VerifierConnexion())
    {
        ...
        else
        {
            ...
            {
                Smartphone sp;
                sp.nom = requete.value("nomSmartphone").toString();
 ...
 * @brief AccesMySQL::ObtenirNomRondeHistorique
 * Cette fonction permet d'interroger la BDD sur le nom des rondes en fonction de l'ID d'une ronde
  ...
  /**/  if(VerifierConnexion())
    {
        ...
        if(requete.exec("SELECT nomrondes FROM rondes INNER JOIN historiquepointeau ON rondes.idRonde = historiquepointeau.idRonde"))
        {
            while (requete.next())
            {
                listeNomRonde.append(requete.value("nomrondes").toString());
            }
        }
        else
        {
            qDebug() <<
                        "Erreur lors de la requête \"SELECT nomrondes FROM rondes INNER JOIN historiquepointeau ON rondes.idRonde = historiquepointeau.idRonde\" : "
                        + requete.lastError().text();
        }
   /**/ }
    qDebug() << "liste des ronde après ObtenirNomRondeHistorique : " << listeNomRonde;
    return listeNomRonde;
}
 ...
 * @author Antoine CHEVREL
 * @brief AccesMySQL::ObtenirNomRondeHistorique
 * Cette fonction permet d'interroger la BDD sur le nom des agents en fonction de l'ID d'un agent
  ...
  /**/  if(VerifierConnexion())
    {
        ...
        if(requete.exec("SELECT nom FROM agents INNER JOIN historiquepointeau ON agents.idAgent = historiquepointeau.idAgent"))
        {
            while (requete.next())
            {
                listeNomAgent.append(requete.value("nom").toString());
            }
        }
        else
        {
            qDebug() <<
                        "Erreur lors de la requête \"SELECT nom FROM agents INNER JOIN historiquepointeau ON agents.idAgent = historiquepointeau.idAgent\" : "
                        + requete.lastError().text();
        }
  /**/  }
    return listeNomAgent;
...
/**
 * @author
 * @brief AccesMySQL::ObtenirRapport
 * Cette fonction permet d'obtenir les informations pour un rapport il interroge la BDD pour mettre en relation un agent, une ronde et/ou une anomalie
 * @param filtreRonde
 * @param filtreAgent
 * @param filtreDate
 * @param nomRonde
   ...
   /**/ QSqlQuery requete(mySqlBdd), requetePointeau(mySqlBdd), requeteMaincourante(mySqlBdd);
    ...
    if(VerifierConnexion())
    {
        ...
        if(filtreRonde)
        {
            if(!filtreAjouter)
            {
                requeteRapport.append(" WHERE '" + nomRonde + "'");
                filtreAjouter = true;
            }
            else
            {
                requeteRapport.append(" AND '" + nomRonde + "'");
            }
        }
        ...
        if(filtreAgent)
        {
            if(!filtreAjouter)
            {
                requeteRapport.append(" WHERE '" + nomAgent + "'");
                filtreAjouter = true;
            }
            else
            {
                requeteRapport.append(" AND '" + nomAgent + "'");
            }
        }
        ...
        if(requete.exec(requeteRapport))
        {
            while(requete.next())
            {
                ...
                if(requetePointeau.exec())
                {
                    ...
                    nvRapport = new Rapport(requetePointeau.value("nom").toString(), requetePointeau.value("nomrondes").toString(), listePointeaux, listeMainscourantes);
